 ...
 * 5:获取用户点赞文章列表
 */
@WebServlet("/mainPage/getLikeArticleList")
public class GetLikeArticleListServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        PageBean<Article> pb = service.pageQuery(userID,currentPage,type,pageSize);

        ResultInfo info=new ResultInfo();
        if (pb.getList()!=null&&pb.getList().size()!=0){
            info.setCode(1);
            info.setMsg("获取点赞文章列表成功");
            info.setCount(pb.getCount());
            info.setData(pb.getList());
        }else {
            info.setCode(0);
            info.setMsg("获取点赞文章列表失败");
        }
